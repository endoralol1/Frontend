...
export function DiscoveryPage() {
  ...
  const [turboFast, setTurboFast] = useState(true)
  ...
  async function load() {
    ...
    setAutoEnabled(String(res.settings.discovery_auto_enabled) === '1')
    setTurboFast(String(res.settings.discovery_turbo_fast ?? '1') === '1')
  }
  ...
  async function saveSettings() {
    ...
    try {
      const res = await api.discoverySave({
        ...
        discovery_auto_enabled: autoEnabled,
        discovery_turbo_fast: turboFast,
      })
  ...
  async function startAuto() {
    ...
    setError('')
    try {
      const res = await api.discoveryStart()
      setAutoEnabled(true)
      setMessage(res.message || 'Automatic puller enabled.')
      await load()
    } catch (e) {
  ...
  async function stopPuller() {
    setBusy(true)
    setMessage('')
    setError('')
    try {
      const res = await api.discoveryStop()
      setAutoEnabled(false)
      setMessage(res.message || 'Puller stopped.')
      await load()
    } catch (e) {
  ...
  const autoOn = String(data.settings.discovery_auto_enabled) === '1'
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Cron ticks every minute when the automatic puller is enabled. Pull now bypasses the timer.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {autoOn ? (
            <Button variant="outline" onClick={stopPuller} disabled={busy}>
              <Square className="h-4 w-4" />
              Stop puller
            </Button>
          ) : (
            <Button variant="outline" onClick={startAuto} disabled={busy}>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Auto puller: {autoOn ? 'Enabled' : 'Stopped'}
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between gap-3 rounded-lg border border-border p-3">
              <div>
                <div className="font-medium">Automatic puller</div>
                <div className="text-xs text-muted-foreground">
                  When off, cron skips discovery until you start it again.
                </div>
              </div>
              <Switch
                checked={autoEnabled}
            ...
            <div className="flex items-center justify-between gap-3 rounded-lg border border-border p-3">
              <div>
                <div className="font-medium">Turbo mode</div>
                <div className="text-xs text-muted-foreground">
                  Faster discovery volume; full scan runs on report open.
                </div>
              </div>
              <Switch checked={turboFast} onCheckedChange={setTurboFast} disabled={busy} />
            </div>
