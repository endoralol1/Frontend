...
<main class="cf-admin page-pad-top">
  <div class="container cf-admin-wrap">
    ...
    <div class="cf-admin-head">
      <h1>Sources</h1>
      <p>Enable/disable, drag order (top = tested first), and probe with any movie/TV TMDB id. Each Test shows how long until a playable stream returns. Users see Alpha/Beta labels.</p>
    <p class="muted" style="margin:.35rem 0 0;font-size:.82rem;color:rgba(255,255,255,.45)">
      Experimental: <b>UpCloud</b> / <b>Vidmoly</b> / <b>Stremify</b> / Railway scrapers (<b>Castle</b>, <b>AwsInd</b>, <b>Nitro</b>, <b>Rive/IP Cloud</b>, <b>HDGHAR</b>, <b>HollyBox</b>) are Vuflix-capable.
      Default audio is <b>English</b>. Use <b>Auto</b> / <b>EN required</b> toggles: if a source has no English, auto-play skips it unless Non-EN auto is on.
    </p>
    </div>
...
(function(){
  ...
  function lastTestBadge(s){
    var lt=s.lastTest; if(!lt) return '';
    var label=lt.elapsedLabel||formatMs(lt.elapsedMs);
    if(!label) return '';
    var ok=!!lt.ok; var play=!!lt.playable;
    var color=play?'#22c55e':(ok?'#f59e0b':'#f87171');
    var tip=(lt.message||'')+' · '+new Date((lt.testedAt||0)*1000).toLocaleString();
    var kind=play?'playable':(ok?'links only':'fail');
    return ' <span title="'+String(tip).replace(/"/g,'&quot;')+'" style="display:inline-block;margin-left:.35rem;padding:.12rem .45rem;border-radius:999px;font-size:.72rem;font-weight:600;background:rgba(255,255,255,.06);color:'+color+';border:1px solid rgba(255,255,255,.1)">⏱ '+label+' · '+kind+'</span>';
  }
  function render(sources){
    list.innerHTML = (sources||[]).map(function(s){
      var autoOn = s.autoLoad !== false;
      var nonEn = !!s.autoloadNonEnglish;
      return '<div class="cf-admin-source'+(s.enabled?'':' is-off')+'" draggable="true" data-id="'+s.id+'">'+
        '<div class="meta"><strong>'+s.name+' <span style="color:rgba(255,255,255,.4)">('+s.id+')</span></strong>'+lastTestBadge(s)+
        '<em>Public: '+s.publicLabel+(s.enabled?' · enabled':' · disabled')+(autoOn?' · auto':' · manual')+(nonEn?' · allow non-EN auto':'')+'</em></div>'+
        '<button type="button" class="cf-admin-switch'+(s.enabled?' on':'')+'" data-toggle="'+s.id+'" aria-label="Toggle enabled" title="Enabled"></button>'+
        '<button type="button" class="cf-admin-btn ghost'+(autoOn?' on':'')+'" data-autoload="'+s.id+'" title="Automatic load in player">'+(autoOn?'Auto':'Manual')+'</button>'+
        '<button type="button" class="cf-admin-btn ghost'+(nonEn?' on':'')+'" data-autone="'+s.id+'" title="Auto-load even when English audio is missing">'+(nonEn?'Non-EN auto':'EN required')+'</button>'+
        '<label style="display:inline-flex;align-items:center;gap:.25rem;font-size:.78rem;color:rgba(255,255,255,.65)" title="Max seconds to wait for the provider API to return stream links">Scrape <input data-timeout="'+s.id+'" type="number" min="15" max="180" step="5" value="'+(s.scrapeTimeoutSec||45)+'" style="width:3.4rem;min-height:2.1rem;border-radius:.55rem;border:1px solid rgba(255,255,255,.12);background:rgba(0,0,0,.28);color:#fff;padding:.25rem .35rem">s</label>'+
        '<label style="display:inline-flex;align-items:center;gap:.25rem;font-size:.78rem;color:rgba(255,255,255,.65)" title="Auto-mode: how long to keep trying this source (playback start) before switching to the next">Auto wait <input data-autowait="'+s.id+'" type="number" min="3" max="120" step="1" value="'+(s.autoWaitSec||15)+'" style="width:3.4rem;min-height:2.1rem;border-radius:.55rem;border:1px solid rgba(255,255,255,.12);background:rgba(0,0,0,.28);color:#fff;padding:.25rem .35rem">s</label>'+
  ...
  list.addEventListener('click', function(e){
    ...
    var a=e.target.closest('[data-autoload]');
    if(a){
      var id=a.getAttribute('data-autoload');
      var on=!a.classList.contains('on');
      fetch(API+'/'+encodeURIComponent(id), {
        method:'POST', credentials:'same-origin',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({autoLoad:on})
      }).then(r=>r.json()).then(function(d){ if(d&&d.ok){ render(d.sources); show(on?'Auto-load on':'Auto-load off (manual only)', true);} else show('Auto-load toggle failed'); });
      return;
    }
    var ne=e.target.closest('[data-autone]');
    if(ne){
      var id=ne.getAttribute('data-autone');
      var on=!ne.classList.contains('on');
      fetch(API+'/'+encodeURIComponent(id), {
        method:'POST', credentials:'same-origin',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({autoloadNonEnglish:on})
      }).then(r=>r.json()).then(function(d){ if(d&&d.ok){ render(d.sources); show(on?'Will auto-load without English':'English required for auto-load', true);} else show('Non-EN auto toggle failed'); });
      return;
    }
    ...
    if(test){
      ...
      fetch(API+'/'+encodeURIComponent(id)+'/test', {
        ...
        var ms = d.elapsedLabel || formatMs(d.elapsedMs);
        var bits = [d.message || (d.ok?'OK':'Fail')];
        if(d.sourceCount!=null) bits.push(d.sourceCount+' streams');
        if(ms) bits.push(ms+(d.playable?' playable':''));
        if(d.scrapeMs!=null && d.probeMs!=null) bits.push('(scrape '+formatMs(d.scrapeMs)+' + probe '+formatMs(d.probeMs)+')');
        show(bits.join(' · '), !!(d.ok && (d.playable!==false || d.playable==null)));
        // Refresh list so last-test badge updates
        load();
  ...
  list.addEventListener('change', function(e){
    ...
    var to=e.target.closest('[data-timeout]');
    if(to){
      var id=to.getAttribute('data-timeout');
      var sec=parseInt(to.value,10)||45;
      if(sec<15) sec=15; if(sec>180) sec=180;
      to.value=sec;
      fetch(API+'/'+encodeURIComponent(id), {
        method:'POST', credentials:'same-origin',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({scrapeTimeoutSec:sec})
      }).then(r=>r.json()).then(function(d){ if(d&&d.ok) show('Scrape timeout saved: '+sec+'s', true); else show('Scrape save failed'); });
      return;
    }
    var aw=e.target.closest('[data-autowait]');
    if(aw){
      var id=aw.getAttribute('data-autowait');
      var sec=parseInt(aw.value,10)||15;
      if(sec<3) sec=3; if(sec>120) sec=120;
      aw.value=sec;
      fetch(API+'/'+encodeURIComponent(id), {
        method:'POST', credentials:'same-origin',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({autoWaitSec:sec})
      }).then(r=>r.json()).then(function(d){ if(d&&d.ok) show('Auto wait saved: '+sec+'s', true); else show('Auto wait save failed'); });
    }
  });
