...
function applyProgressiveMp4ContentType(headers: Headers, upstreamUrl: string) {
    const lower = upstreamUrl.toLowerCase()
    const looksProgressiveMp4 =
...
async function handleProxy(request: Request) {
    ...
    try {
        ...
        if (
            ...
            applyProgressiveMp4ContentType(headers, upstreamUrl)
