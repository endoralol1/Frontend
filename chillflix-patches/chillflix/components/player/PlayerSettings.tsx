...
interface PlayerSettingsProps {
    ...
    onSubtitleDelayChange?: (delaySec: number) => void
    subtitleFontScale?: number
    onSubtitleFontScaleChange?: (scale: number) => void
                    ...
                    <div className={cn(SETTINGS_SCROLL, "mt-2 pr-0.5")}>
                                        ...
                                        {unavailable ? (
                                            ...
                                            <div className="space-y-0.5">
                                                ...
                                                {providerSources.map((source) => {
                                                    ...
                                                    // Qualities live in the Quality tab — Source is one row per provider.
                                                    const streamLabel = displayName
                        ...
                        <TabsContent value="subtitles" className="m-0 space-y-2 px-1 pb-1">
                            {(onSubtitleDelayChange ||
                                onSubtitleFontScaleChange ||
                                onSubtitleBackgroundOpacityChange) && (
                                <div className={cn(SETTINGS_PANEL, "space-y-2.5 p-2.5")}>
                                    {onSubtitleDelayChange ? (
                                        <div className="space-y-1.5">
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-[11px] font-medium text-zinc-400">
                                                    {t("player.subtitleDelay")}
                                                </span>
                                                <span className="text-[11px] font-semibold tabular-nums text-white">
                                                    {subtitleDelaySec > 0 ? "+" : ""}
                                                    {subtitleDelaySec.toFixed(1)}s
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-1.5">
                                                <Button
                                                    ...
                                                    variant="outline"
                                                    size="sm"
                                                    className="h-7 border-zinc-700 px-2 text-[11px] text-zinc-200"
                                                    onClick={() =>
                                                        onSubtitleDelayChange(
                                                            Math.max(-10, Math.round((subtitleDelaySec - 0.5) * 10) / 10)
                                                        )
                                                    }
                                                >
                                                    −0.5s
                                                ...
                                                <input
                                                    ...
                                                    max={10}
                                                    step={0.1}
                                                    value={subtitleDelaySec}
                                                    onChange={(event) =>
                                                        onSubtitleDelayChange(Number(event.target.value))
                                                    }
                                                    className="h-1.5 w-full cursor-pointer accent-primary"
                                                    aria-label={t("player.subtitleDelay")}
                                                />
                                                <Button
                                                    type="button"
                                                    variant="outline"
                                                    ...
                                                    onClick={() =>
                                                        onSubtitleDelayChange(
                                                            Math.min(10, Math.round((subtitleDelaySec + 0.5) * 10) / 10)
                                                        )
                                                    }
                                                >
                                                    +0.5s
                                    ...
                                    {onSubtitleFontScaleChange ? (
                                        <div className="space-y-1.5">
                                            ...
                                            <input
                                                ...
                                                onChange={(event) =>
                                                    onSubtitleFontScaleChange(Number(event.target.value))
                                                }
                                                className="h-1.5 w-full cursor-pointer accent-primary"
                                    ...
                                    {onSubtitleBackgroundOpacityChange ? (
                                        <div className="space-y-1.5">
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                <span className="text-[11px] font-semibold tabular-nums text-white">
                                                    {Math.round(subtitleBackgroundOpacity * 100)}%
                                                </span>
                                            </div>
                                            <input
                                                type="range"
                                                ...
                                                onChange={(event) =>
                                                    onSubtitleBackgroundOpacityChange(Number(event.target.value))
                                                }
                                                className="h-1.5 w-full cursor-pointer accent-primary"
                                                aria-label={t("player.subtitleBackground")}
                                            />
                                        </div>
                                    ) : null}
                                </div>
