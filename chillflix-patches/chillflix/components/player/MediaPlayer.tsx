...
export function MediaPlayer({
  ...
  useEffect(() => {
    ...
    const isHollyProvider = normalizeProviderName(providerKey) === "hollymoviehd"
    const effectiveSourceLoadTimeoutMs =
      isHollyProvider || isProgressiveProxyMp4
        ? Math.max(sourceLoadTimeoutMs, 45_000)
        : sourceLoadTimeoutMs
    const effectiveStartupFailMs =
      isHollyProvider || isProgressiveProxyMp4
        ? Math.max(startupFailMs, 60_000)
        : startupFailMs
