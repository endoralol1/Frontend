...
import {
    ...
    shouldHoldEmptyStateForDiscovery,
...
type UseMediaSourcesParams = PrefetchMediaSourcesParams & {
  ...
  /**
   * Updated by the player while a scan is in flight. When a health probe marks
   * any source ready, the walk stops immediately (first working stream wins).
   */
  hasReadySourceRef?: { current: boolean }
  /**
   * Provider ids whose streams all failed health probes — settle waits exit
   * early so the next admin-ordered provider is fetched without a long delay.
   */
  healthFailedProviderIdsRef?: { current: readonly string[] }
...
const BACKGROUND_PENDING_CAP_MS = 28_000
const INITIAL_LOADING_CAP_MS = 18_000
...
/** Auto-scan per-provider budget (legacy fallback). */
const AUTO_PROVIDER_SCAN_TIMEOUT_MS = 8_000
/**
 * Multi-pass discovery (3 circles through admin order).
 * Soft timeouts per pass — early passes must not permanently skip slow scrapers.
 * Final pass allows cold Flixhqz-class resolves (~30s+).
 */
const SCAN_PASS_TIMEOUTS_MS = [15_000, 28_000, 45_000] as const
const SCAN_PASS_COUNT = 3
/** Keep #1 alive longer than secondaries so a slow primary is not false-failed. */
const SCAN_PRIMARY_TIMEOUT_BONUS_MS = 20_000
/** After #1 head-start, fetch this many secondaries at once. */
const SCAN_SECONDARY_CONCURRENCY = 6
...
export function useMediaSources({
        ...
        try {
          localStorage.setItem(
            ...
            JSON.stringify({
              ...
              // Keep browser cache aligned with Chillflix source cache (15m) so
              // reopening the same title does not force a full provider rescan.
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        // Mark scanning now so the player cannot flash the empty-state panel
        // in the gap before the scan effect starts.
        if (!isStale()) {
          setResolvedParamsKey(cacheKey)
          setIsScanningProviders(true)
          setIsRefreshing(true)
  ...
  const backgroundPendingActive =
    enabled &&
    sources.length === 0 &&
    shouldHoldEmptyStateForDiscovery(diagnostics, sources.length)
  ...
  useEffect(() => {
    ...
    const runProviderScan = async () => {
      ...
      // Clear stale ready/failed flags from the previous title so this scan
      // actually runs (a leftover hasReadySourceRef would no-op the walk).
      if (hasReadySourceRef) hasReadySourceRef.current = false
      if (healthFailedProviderIdsRef) {
      ...
      // Stop when playback is stable OR a health probe already found a working
      // stream. Do NOT stop on mere URL presence (dead links).
      const shouldStopDiscovering = () =>
        cancelled ||
        playbackActiveRef.current ||
        Boolean(hasReadySourceRef?.current)

      const providerProbeFailed = (providerId: string) => {
        const failed = healthFailedProviderIdsRef?.current
        if (!failed || failed.length === 0) return false
        const key = normalizeProviderName(providerId)
        return failed.some((id) => normalizeProviderName(id) === key)
      }

      // Cap how long we wait for probe/auto-start after a provider returns URLs.
      // Full client probe timeout is 20s — waiting that per provider made cold
      // starts feel broken. Probe-ready / probe-failed exits early below.
      const PROVIDER_SETTLE_MS = Math.min(
        Math.max(getHeadStartMs(), 3_000),
        6_000
      )
      ...
      const tryProvider = async (
        ...
        // Playback started or a probe already found a working stream — stop
        // spending resolves. Dead URLs must NOT stop the walk.
        if (playbackActiveRef.current || hasReadySourceRef?.current) {
      ...
      // Multi-pass discovery (3 circles through admin order):
      // - Pass 1: head-start on #1, then overlap secondaries so a fast Huhu
      //   is not stuck behind a 30s Flixhqz. Soft-fail timeouts (retry later).
      // - Pass 2–3: force-fresh retries for providers still missing.
      // Stop as soon as a health probe marks a stream ready.
      const waitForProbeSettle = async (providerId?: string) => {
        const settleDeadline = Date.now() + PROVIDER_SETTLE_MS
        while (Date.now() < settleDeadline) {
          if (shouldStopDiscovering()) return
          if (providerId && providerProbeFailed(providerId)) return
          if (
            hasPlayableAutomaticSource() &&
            hasReadySourceRef?.current
          ) {
            return
          }
          await sleep(100)
      ...
      try {
        ...
        for (let pass = 0; pass < SCAN_PASS_COUNT; pass += 1) {
          if (shouldStopDiscovering()) break

          const timeoutMs =
            SCAN_PASS_TIMEOUTS_MS[
              Math.min(pass, SCAN_PASS_TIMEOUTS_MS.length - 1)
            ] ?? 20_000
          const passBudgetStartedAt = Date.now()
          const forceFresh = pass > 0

          const pending = scanIds.filter(
            (providerId) =>
              !sourcesIncludeProvider(sourcesRef.current, providerId)
          )

          // Everything already has URLs — wait for a probe-ready pick.
          // If nothing probes healthy, force-fresh the unhealthy set next circle
          // instead of treating "has URL" as "has source".
          if (pending.length === 0) {
            await waitForProbeSettle(scanIds[0])
            if (shouldStopDiscovering()) break
            if (pass < SCAN_PASS_COUNT - 1) {
              for (const providerId of scanIds) {
                scanFailedRef.current.delete(providerId)
              }
              setScanFailedProviderIds([...scanFailedRef.current])
              // Circle again: prefer probe-failed providers, else re-scrape the
              // top of admin order so we do not give up on a title that only
              // returned dead/stale links on the first hit.
              const probeFailed = scanIds.filter((providerId) =>
                providerProbeFailed(providerId)
              )
              const retryIds =
                probeFailed.length > 0 ? probeFailed : scanIds.slice(0, 4)
              for (const providerId of retryIds) {
                if (shouldStopDiscovering()) break
                setScanningProviderId(providerId)
                ...
                if (sourcesIncludeProvider(sourcesRef.current, providerId)) {
                  await waitForProbeSettle(providerId)
                }
                if (shouldStopDiscovering()) break
              }
            }
          ...
          const primaryId = pending[0]
          const secondaryIds = pending.slice(1)

          // Keep #1 request in flight; do not abandon it when overlapping.
          setScanningProviderId(primaryId)
          const primaryTimeoutMs = Math.min(
            timeoutMs + SCAN_PRIMARY_TIMEOUT_BONUS_MS,
            SCAN_PASS_TIMEOUTS_MS[SCAN_PASS_TIMEOUTS_MS.length - 1]
          )
          const primaryPromise = tryProvider(primaryId, {
            useFetchTimeout: true,
            // Soft-fail until the final pass so timeouts can be retried.
            markFailedOnMiss: pass === SCAN_PASS_COUNT - 1,
            forceFresh,
            scanTimeoutMs: primaryTimeoutMs,
          })

          // Head-start exclusive window for admin #1 (pass 1 only).
          if (pass === 0) {
            const headStartDeadline = Date.now() + Math.max(getHeadStartMs(), 1_000)
            while (Date.now() < headStartDeadline) {
              if (shouldStopDiscovering()) break
              if (sourcesIncludeProvider(sourcesRef.current, primaryId)) break
              await sleep(100)
            }
          }
          ...
          // Fire secondaries in parallel (capped concurrency) so a fast
          // provider like Huhu is not stuck behind a 25–30s MovieBox/OnlyFlix
          // scrape. A watcher settles/probes as soon as any URL appears.
          if (secondaryIds.length > 0) {
            setScanningProviderId(secondaryIds[0] ?? primaryId)

            const queue = [...secondaryIds]
            const active = new Set<Promise<boolean>>()

            const launchNext = () => {
              while (
                active.size < SCAN_SECONDARY_CONCURRENCY &&
                queue.length > 0 &&
                !shouldStopDiscovering()
              ) {
                const providerId = queue.shift()!
                const task = tryProvider(providerId, {
                  useFetchTimeout: true,
                  markFailedOnMiss: pass === SCAN_PASS_COUNT - 1,
                  forceFresh,
                  scanTimeoutMs: timeoutMs,
            ...
            const watchForReady = (async () => {
              while (!shouldStopDiscovering()) {
                if (hasPlayableAutomaticSource()) {
                  await waitForProbeSettle()
                  if (shouldStopDiscovering()) return
                }
          ...
          // Primary may still be resolving — await it before the next pass.
          const primaryFound = await primaryPromise
          if (primaryFound || sourcesIncludeProvider(sourcesRef.current, primaryId)) {
            clearProviderFailed(primaryId)
            await waitForProbeSettle(primaryId)
          }
          ...
          // If providers returned empty instantly, do NOT advance the circle
          // immediately — keep the pass window open so slow scrapers (and
          // in-flight primary resolves) can still land before we soft-retry.
          if (!shouldStopDiscovering() && !hasPlayableAutomaticSource()) {
            const passStartedAt = passBudgetStartedAt
            const remaining = timeoutMs - (Date.now() - passStartedAt)
            const waitUntil = Date.now() + Math.max(0, remaining)
            while (Date.now() < waitUntil && !shouldStopDiscovering()) {
              if (hasPlayableAutomaticSource()) {
                await waitForProbeSettle()
                break
              }
          ...
          // Prepare soft retries: clear failed marks so the next circle
          // can force-fresh providers that timed out or returned empty.
          if (pass < SCAN_PASS_COUNT - 1 && !shouldStopDiscovering()) {
            for (const providerId of scanIds) {
              if (!sourcesIncludeProvider(sourcesRef.current, providerId)) {
                scanFailedRef.current.delete(providerId)
              }
  ...
  return {
    ...
    isLoadingMore:
      // Keep "loading more" true for the whole multi-pass scan so the empty
      // state does not flash while Flixhqz is still cold-scraping — and while
      // a soft CinePro timeout races a late source that is about to land.
      isScanningProviders ||
