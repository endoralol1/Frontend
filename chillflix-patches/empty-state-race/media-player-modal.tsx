...
export const MediaPlayerModal: React.FC<MediaPlayerModalProps> = ({
  ...
  const skipHealthProbe = useMemo(
    () =>
      ...
      shouldSkipPlaybackHealthProbe({
        ...
        // Always probe on site player — matching watch-page admin-order testing.
  ...
  const trustedProbeSourceIds = useMemo(() => {
    ...
    // Site player: probe in admin order. Do not pre-trust proxy URLs or a
    // lower-ranked ready mark will abort probing of #1.
    if (!skipHealthProbe) {
      return [] as string[]
    }
    ...
    () =>
      selectedPlayback
        ? {
            ...
            qualities: selectedPlayback.qualities,
          }
        : null,
    [
      selectedPlayback?.id,
  ...
  // Also hold while soft CinePro timeouts race a late source (empty-state flash).
  ...
  const showSourceLoading =
    ...
    // Never flash “title isn’t available” while more sources may still land.
    (!playerSource && sourcesLoadingMore)
  ...
  const sourceStatusMessage = useMemo(() => {
    ...
    return (
      ...
      // Soft timeout for *extra* CinePro providers — ignore once a stream exists.
      (hasPlayable
        ? undefined
