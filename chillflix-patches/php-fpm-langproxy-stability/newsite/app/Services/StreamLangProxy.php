...
final class StreamLangProxy
{
    ...
    {
        // Playlists are small text; segments can be 50–100MB+ and must NOT be buffered in PHP RAM.
        $tmp = tempnam(sys_get_temp_dir(), 'cfhls_');
        if ($tmp === false) {
            http_response_code(500);
            exit;
        }
        try {
            $meta = self::httpDownloadToFile($url, $tmp, $headers, 120 * 1024 * 1024);
            ...
            // Playlist rewrite needs the full (small) text body.
            if (filesize($tmp) > 8 * 1024 * 1024) {
                http_response_code(502);
                header('Content-Type: application/json');
                echo json_encode(['ok' => false, 'error' => 'Playlist too large']);
                exit;
            }
    ...
    {
        $tmp = tempnam(sys_get_temp_dir(), 'cfget_');
        if ($tmp === false) {
            return null;
        }
        try {
