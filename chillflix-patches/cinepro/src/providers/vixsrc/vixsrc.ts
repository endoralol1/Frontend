...
/** Same Cloudflare Worker as Yoru (`YORU_RELAY_URL`) — exposes GET /vixsrc. */
function resolveVixSrcRelay(): { base: string; secret: string } | null {
    const base = (
        process.env.VIXSRC_RELAY_URL?.trim() ||
        process.env.YORU_RELAY_URL?.trim() ||
        ''
    ).replace(/\/$/, '');
    const secret =
        process.env.VIXSRC_RELAY_SECRET?.trim() ||
        process.env.YORU_RELAY_SECRET?.trim() ||
        '';
...
export class VixSrcProvider extends BaseProvider {
    ...
    /**
     * Prefer the shared Yoru Cloudflare Worker (`/vixsrc`) when the VPS IP is CF-banned.
     */
    private async getSourcesViaRelay(
        media: ProviderMediaObject
    ): Promise<ProviderResult | null> {
        const relay = resolveVixSrcRelay();
        if (!relay) return null;

        const type = media.type === 'tv' ? 'tv' : 'movie';
        const params = new URLSearchParams({
            type,
            tmdb: String(media.tmdbId ?? ''),
            season: String(media.s ?? 1),
            episode: String(media.e ?? 1),
            key: relay.secret
        });

        try {
            const response = await fetch(`${relay.base}/vixsrc?${params}`, {
                headers: {
                    Accept: 'application/json',
                    'x-yoru-key': relay.secret
                },
            ...
            if (!response.ok || !payload?.ok || !payload.sources?.length) {
                return this.emptyResult(
                    payload?.error
                        ? `Relay: ${payload.error}`
                        : `Relay failed (HTTP ${response.status})`,
                    media
                );
            }

            const pageUrl = this.buildPageUrl(media);
            const referer = payload.referer || pageUrl;
            const sources: Source[] = [];

            for (const src of payload.sources) {
                if (!src?.url) continue;
                sources.push({
                    url: this.createProxyUrl(src.url, {
                        ...this.HEADERS,
                        Referer: referer
                    }),
                    type: 'hls',
            ...
            if (!sources.length) {
                return this.emptyResult('Relay returned no usable streams', media);
            }
    ...
    private async getSources(
        ...
        try {
            // CF-banned VPS: use shared Worker relay first (same as Yoru).
            if (resolveVixSrcRelay()) {
                const viaRelay = await this.getSourcesViaRelay(media);
    ...
    private describeFetchFailure(status: number): string {
        if (status === 403 || status === 401) {
            ...
            if (!resolveVixSrcRelay()) {
                hints.push(
                    'Deploy /vixsrc on the Yoru Cloudflare Worker and set YORU_RELAY_URL + YORU_RELAY_SECRET (or VIXSRC_RELAY_*).'
                );
            }
            if (!resolveVixSrcProxyUrl()) {
                hints.push(
                    'Or set VIXSRC_PROXY to a residential HTTP proxy / FlareSolverr (FLARESOLVERR_URL).'
                );
                ...
                hints.push(
                    'Server and proxy IPs are banned by Cloudflare. Use the Worker /vixsrc relay or a residential proxy.'
                );
            }
