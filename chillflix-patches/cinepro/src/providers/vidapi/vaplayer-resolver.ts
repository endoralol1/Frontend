...
function relayConfig() {
    const base = (process.env.YORU_RELAY_URL || process.env.VAPLAYER_RELAY_URL || '').replace(
        /\/$/,
        ''
    );
    const secret =
        process.env.YORU_RELAY_SECRET || process.env.VAPLAYER_RELAY_SECRET || '';
...
async function fetchViaWorker(
    ...
    const url = new URL(`${cfg.base}/vaplayer`);
    url.searchParams.set('type', type);
...
export async function resolveVaplayerStreams(
    ...
    // Prefer Worker when available (escape DC rate-limits), then direct.
    const preferWorker =
    ...
    // If Worker was skipped or failed and direct failed, surface Worker error if any.
    if (!preferWorker) {
        const viaWorker = await fetchViaWorker(media);
        if (viaWorker?.streams.length) return viaWorker;
