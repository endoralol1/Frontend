...
<div class="np-shell ${cfg.embed ? "np-embed" : ""}" id="np-shell">
  ...
  function createPlayer(cfg) {
      ...
      if (isHls && window.Hls && window.Hls.isSupported()) {
        ...
        hls.on(window.Hls.Events.ERROR, (_e, data) => {
          ...
          const hardMiss =
            ...
            ((httpCode === 403 || /not found|404/i.test(String(data.response?.text || data.reason || ""))) &&
    ...
    async function ensureProviderLoaded(index, { play = false, isAuto = false, background = false } = {}) {
    ...
    async function runBackgroundSourceSearch() {
      ...
      refreshMenus();
      try {
        await ensureProviderLoaded(0, { play: true, isAuto: true, background: true });
      } catch (_) {
    ...
    function armLoadWatchdog(source) {
      ...
      const viaProxy = /\/api\/player\/(media-proxy|lang-proxy)\b/i.test(abs);
