(function () {
  ...
  function hardStopVideo(video) {
    ...
    try {
      ...
      try { video.src = ""; } catch (eSrc) {}
      try { video.srcObject = null; } catch (eSo) {}
      video.load();
    } catch (e2) {}
    try { video.classList.remove("is-ready"); } catch (e3) {}
  }
  ...
  function stopCard(card) {
    ...
    hardStopVideo(video);
    try { card.dataset.cfMuted = "1"; } catch (eM) {}
    try {
      var muteIcon = card.querySelector(".cf-card-mute i");
      if (muteIcon) muteIcon.className = "uil uil-volume-mute";
    } catch (eI) {}
  ...
  /**
   * Kill every card preview video — not just activeCard.
   * Softnav swaps .wrapper via innerHTML; removing a playing unmuted <video>
   * without pause() can leave ghost audio (intermittent, Chrome/Safari).
   * Must run on cf:before-softnav (before DOM wipe) and before Watch navigates.
   */
  function killAllPreviews() {
    ...
    touchPreview = false;
    if (activeCard) {
      try { stopCard(activeCard); } catch (eA) {}
      activeCard = null;
    }
    try {
      document.querySelectorAll("video.cf-card-video").forEach(function (video) {
        hardStopVideo(video);
        var card = video.closest && video.closest(".movie-item");
        if (card) {
          try { card.dataset.cfMuted = "1"; } catch (eM) {}
          card.classList.remove("is-hover-preview", "has-preview-media", "is-touch-preview");
          try {
  ...
  try {
    window.__cfKillCardPreviews = killAllPreviews;
  } catch (eExp) {}
  ...
  function bind() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (shield) {
        ...
        if (link && link.href) {
          e.preventDefault();
          e.stopPropagation();
          // Pause/mute before navigation — prevents ghost audio during full page load / bfcache
          killAllPreviews();
      ...
      // Play CTA / poster link leaving the page — kill unmuted preview first
      var playNav = e.target && e.target.closest
        ? e.target.closest(".cf-card-play, a.item-poster")
        : null;
      if (playNav && playNav.href && !playNav.classList.contains("cf-card-watchlist")) {
        try {
          var navUrl = new URL(playNav.href, window.location.href);
          if (navUrl.origin === window.location.origin && /\/(movie|tv|player)\//.test(navUrl.pathname || "")) {
            killAllPreviews();
          }
        } catch (eNav) {
          killAllPreviews();
    ...
    document.addEventListener("visibilitychange", function () {
      if (document.hidden) killAllPreviews();
    });

    // Before softnav wipes .wrapper — pause unmuted videos first (ghost-audio fix)
    window.addEventListener("cf:before-softnav", killAllPreviews);
    window.addEventListener("cf:softnav", killAllPreviews);
    window.addEventListener("pagehide", killAllPreviews);
    window.addEventListener("beforeunload", killAllPreviews);
