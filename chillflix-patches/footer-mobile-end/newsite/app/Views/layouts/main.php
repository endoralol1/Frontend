...
<head>
    ...
    <link rel="stylesheet" href="<?= e(asset('css/app-docksearch-1248.css')) ?>?v=20260816-footer-end1">
...
<script>
    ...
    $cfAdsLib = is_array($cfAdsHead) ? (string) ($cfAdsHead['antiAdblockSrc'] ?? '/assets/js/k7m2qx9p.js') : '/assets/js/k7m2qx9p.js';
    if ($cfAdsHeadNeed):
    ?>
    <script>
...
<div class="wrapper">
    ...
    <?php require $content; ?>
    <?php
    $cfAds = class_exists('SiteAds') ? SiteAds::get() : [];
    $cfAdsPath = function_exists('current_path') ? current_path() : '/';
    $cfAdsPageOk = $cfAds && SiteAds::shouldRenderOnPage($bodyClass ?? null, $cfAdsPath);
    $cfAdsBannerOk = $cfAdsPageOk && SiteAds::unitOn('banner');
    $cfAdsInterstitialOk = $cfAdsPageOk && SiteAds::unitOn('interstitial');
    $cfAdsVideoOk = $cfAdsPageOk && SiteAds::unitOn('videoSlider');
    $cfAdsPopOk = $cfAds && SiteAds::popShouldRender($bodyClass ?? null, $cfAdsPath);
    $cfAdsPopBypass = $cfAds && !empty($cfAds['popBypassUserOptOut']);
    $cfAdsNeedScript = $cfAdsBannerOk || $cfAdsInterstitialOk || $cfAdsVideoOk || $cfAdsPopOk;
    if ($cfAdsNeedScript):
        // Anti-adblock lib already loaded in <head>; fallback CDN only if missing.
        $cfAdsLibSrc = (string) ($cfAds['antiAdblockSrc'] ?? '/assets/js/k7m2qx9p.js');
    ?>
    <!-- AdCash units (admin: /admin/ads) -->
    <script>
    window.CF_ADS = Object.assign(window.CF_ADS || {}, <?= json_encode([
        'banner' => (bool) $cfAdsBannerOk,
        'interstitial' => (bool) $cfAdsInterstitialOk,
        'videoSlider' => (bool) $cfAdsVideoOk,
        'pop' => (bool) $cfAdsPopOk,
        'popBypassUserOptOut' => (bool) $cfAdsPopBypass,
        'bannerZone' => (string) ($cfAds['bannerZone'] ?? ''),
        'interstitialZone' => (string) ($cfAds['interstitialZone'] ?? ''),
        'videoSliderZone' => (string) ($cfAds['videoSliderZone'] ?? ''),
        'popZone' => (string) ($cfAds['popZone'] ?? ''),
    ], JSON_UNESCAPED_SLASHES) ?>);
    (function () {
      if (window.aclib) return;
      var s = document.createElement('script');
      s.id = 'aclib';
      s.src = <?= json_encode($cfAdsLibSrc, JSON_UNESCAPED_SLASHES) ?>;
      document.head.appendChild(s);
    })();
    </script>
    <?php if ($cfAdsPopOk): ?>
    <script type="text/javascript">
        (function runPopWhenReady(n) {
          // User Settings can disable Pop unless admin bypass is on.
          try {
            var bypass = !!(window.CF_ADS && window.CF_ADS.popBypassUserOptOut);
            if (!bypass) {
              var v = null;
              try { v = localStorage.getItem('cf_pref_ads'); } catch (e0) {}
              if (v == null || v === '') {
                v = /(?:^|;\s*)cf_pref_ads=0(?:;|$)/.test(document.cookie || '') ? '0' : '1';
              }
              if (v === '0') return;
            }
          } catch (e1) {}
          if (window.aclib && typeof aclib.runPop === 'function') {
            aclib.runPop({ zoneId: <?= json_encode((string) $cfAds['popZone'], JSON_UNESCAPED_SLASHES) ?> });
            return;
          }
          if (n < 40) setTimeout(function () { runPopWhenReady(n + 1); }, 100);
        })(0);
    </script>
    <?php endif; ?>
    <?php if ($cfAdsInterstitialOk): ?>
    <script type="text/javascript">
        (function runIntWhenReady(n) {
          if (!(window.CF_ADS && window.CF_ADS.interstitial)) return;
          if (window.aclib && typeof aclib.runInterstitial === 'function') {
            aclib.runInterstitial({ zoneId: <?= json_encode((string) $cfAds['interstitialZone'], JSON_UNESCAPED_SLASHES) ?> });
            return;
          }
          if (n < 40) setTimeout(function () { runIntWhenReady(n + 1); }, 100);
        })(0);
    </script>
    <?php endif; ?>
    <?php if ($cfAdsVideoOk): ?>
    <script type="text/javascript">
        (function runVsWhenReady(n) {
          if (!(window.CF_ADS && window.CF_ADS.videoSlider)) return;
          if (window.aclib && typeof aclib.runVideoSlider === 'function') {
            aclib.runVideoSlider({ zoneId: <?= json_encode((string) $cfAds['videoSliderZone'], JSON_UNESCAPED_SLASHES) ?> });
            return;
          }
          if (n < 40) setTimeout(function () { runVsWhenReady(n + 1); }, 100);
        })(0);
    </script>
    <?php endif; ?>
    <?php if ($cfAdsBannerOk): ?>
    <div class="container" id="cf-adcash-banner" data-cf-ad-unit="banner">
        <script type="text/javascript">
            (function runBannerWhenReady(n) {
              if (!(window.CF_ADS && window.CF_ADS.banner)) return;
              if (window.aclib && typeof aclib.runBanner === 'function') {
                aclib.runBanner({ zoneId: <?= json_encode((string) $cfAds['bannerZone'], JSON_UNESCAPED_SLASHES) ?> });
                return;
              }
              if (n < 40) setTimeout(function () { runBannerWhenReady(n + 1); }, 100);
            })(0);
        </script>
    </div>
    <?php endif; ?>
...
<link rel="stylesheet" href="<?= e(asset('css/home-personalize.css')) ?>?v=20260816-rail-sync1">
...
<script src="<?= e(asset('js/home-personalize.js')) ?>?v=20260816-rail-sync1" defer></script>
...
<script src="<?= e(asset('js/app.docksearch-1236.js')) ?>?v=20260816-rail-sync1" defer></script>
