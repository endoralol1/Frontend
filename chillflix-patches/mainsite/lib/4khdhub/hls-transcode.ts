...
export function getOrStartHlsSession(
  key: string,
  sourceUrl: string,
  mode: HlsSessionMode = "remux"
): HlsTranscodeSession {
  cleanupExpiredSessions()

  // Server CPU guard: never start libx264 re-encode path.
  // Remux (-c:v copy) remains available when FOUR_K_REMUX_* allows it.
  if (mode === "transcode") {
    throw new Error(
      "4K transcoding is disabled on this server to protect CPU. Use remux or 1080p."
    )
  }
