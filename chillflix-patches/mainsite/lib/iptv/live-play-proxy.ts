...
async function fetchResolvedMediahubManifest(playPageUrl: string) {
  const streamUrl = await resolveMediahubStream(playPageUrl)
  const result = await fetchWithRedirects(streamUrl, {
    Referer: `${new URL(playPageUrl).origin}/`,
  })
...
export async function resolveLivePlayManifest(
  ...
  const mediahubCandidates = [
    playPageUrl,
    buildMediahubPlayPageUrl(channelId),
  ].filter((value, index, list): value is string => Boolean(value && list.indexOf(value) === index))

  for (const candidate of mediahubCandidates) {
    try {
      const result = await fetchResolvedMediahubManifest(candidate)
      if (!result) continue
