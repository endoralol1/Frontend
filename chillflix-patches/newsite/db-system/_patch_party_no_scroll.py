#!/usr/bin/env python3
"""Make Watch Party modal fit without sheet scroll; hide form after create."""
...
css = css.replace(
  ...
  /* THIS is the spacing from the inner header edges */
  padding: 16px 18px;
}
.cf-party-copy {
  min-width: 0;
...
# Reset created state when opening / switching tabs
...
new_switch = """  function resetCreatePane() {
...
if old_switch not in js:
    raise SystemExit("switchPartyTab not found")
...
if old_open not in js:
    raise SystemExit("openPartyPanel not found")
js = js.replace(old_open, new_open, 1)

old_success = """      resetPartyTurnstile("create");
      $("#cf-party-created")
        .prop("hidden", false)
        .html(
...
new_success = """      resetPartyTurnstile("create");
      $("#cf-party-results").empty();
      $("#cf-party-q").val("");
...
if old_success not in js:
    raise SystemExit("create success block not found")
...
lt = layout.read_text()
layout.write_text(re.sub(r"\?v=20260803-ui\d+", "?v=20260803-ui145", lt))
print("layout ui145")
print("DONE")
