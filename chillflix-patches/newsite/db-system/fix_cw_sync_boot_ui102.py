...
REMOTE = Path(__file__).with_name("_remote_cw_sync_boot_ui102.py")
if not PW:
...
run(
    ["sshpass", "-e", "ssh", "-o", "StrictHostKeyChecking=no", f"root@{HOST}", "cat > /tmp/ns-cw-ui102.py"],
    input_text=REMOTE.read_text(),
)
run(["sshpass", "-e", "ssh", "-o", "StrictHostKeyChecking=no", f"root@{HOST}", "python3 /tmp/ns-cw-ui102.py"])
print("DEPLOYED ui102")
