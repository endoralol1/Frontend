  ...
  function createPlayer(cfg) {
    ...
    function refreshMenus() {
      renderList(
        ...
        (s, i) => {
          ...
          else if (status === "loading") sub = "Loading…";
          else if (status === "error") sub = "Failed — tap retry";
    ...
    async function ensureProviderLoaded(index, { play = false, isAuto = false } = {}) {
      ...
      const run = (async () => {
        ...
        q.set("provider", pid);
        const res = await fetch(`${sourcesApiBase()}?${q.toString()}`, {
          headers: { Accept: "application/json" },
          credentials: "same-origin",
        });
        const data = await res.json();
        ...
        // Server collapses VAPlayer → one entry; take first matching provider.
        const found =
          data.sources.find((s) => String(s?.provider || "").toLowerCase() === pid) || data.sources[0];
        const keepName = slot.providerName;
        const keepPublic = slot.publicLabel;
        Object.assign(slot, found, {
          provider: pid,
          providerName: found.providerName || keepName,
          publicLabel: found.publicLabel || keepPublic,
          status: "ready",
        });
        if (Array.isArray(data.subtitles) && data.subtitles.length) {
          state.payloadSubtitles = data.subtitles;
        }
        refreshMenus();
        return true;
      })();
      ...
      try {
        ...
        slot.status = "error";
        slot.error = err?.message || "Failed";
        refreshMenus();
        if (isAuto || play) {
          // Auto / failure path: try next admin-ordered source (don't scrape all up front).
          const next = idx + 1;
          if (next < state.sources.length) {
            setStatus(`${slot.providerName || pid} failed — trying next…`);
            return ensureProviderLoaded(next, { play: true, isAuto: true });
          }
          setStatus(err?.message || "No playable sources");
        } else {
          setStatus(err?.message || "Source failed");
        }
        return false;
      } finally {
