...
<main class="watch-p"<?= !empty($backdrop) ? ' style="--cf-watch-backdrop:url(\'' . e($backdrop) . '\')"' : '' ?>>
    ...
    (function () {
      ...
      function startIfNeeded(opts) {
        opts = opts || {};
        if (!window.PLAYER) return;
        var wrap = document.querySelector('.watch-wrap');
        var forced = wrap && wrap.getAttribute('data-autoplay') === '1';
        // Page load: always mount player + search sources.
        // Only autoplay video when Auto Play pref / ?play=1 / explicit forcePlay.
        if (opts.forcePlay === true) {
          window.PLAYER.autoplay = true;
        } else if (opts.forcePlay === false) {
          window.PLAYER.autoplay = false;
        } else if (typeof opts.wantPlay === 'boolean') {
          window.PLAYER.autoplay = opts.wantPlay;
        } else {
          window.PLAYER.autoplay = !!(readPref() || forced);
        }
        if (document.getElementById('np-shell') && !opts.remount) return;
        try {
        ...
        var t = setInterval(function () {
          tries += 1;
          if (go() || tries > 40) clearInterval(t);
        }, 100);
      }
      function toggle(e) {
        ...
        if (on) {
          ...
          // Remount with autoplay so toggling ON after eager mount actually plays
          startIfNeeded({ forcePlay: true, remount: true });
        }
      }
      function bind() {
        var btn = document.getElementById('btn-autoplay');
        ...
        sync();
        // Always mount player + start source search on watch page load.
        // Auto Play pref only controls whether video starts playing.
        var forced = false;
        var wrap = document.querySelector('.watch-wrap');
        if (wrap) forced = wrap.getAttribute('data-autoplay') === '1';
        var wantPlay = !!(readPref() || forced);
        setTimeout(function () {
          startIfNeeded({ wantPlay: wantPlay, remount: false });
        }, 150);
      }
    ...
    <div class="container watch-wrap cf-watch-content"
        ...
        <div class="aside-wrapper swap">
            <aside class="main">
<div id="movie-player" class="no-player playable cf-eager-player" style="position:relative;">
<div class="movie-player-wrap">
                        ...
                        <div id="player">
                            ...
                            <div class="cf-player-boot" id="cf-player-boot" aria-live="polite">
                                <span><?= e(t('player.loading_sources', [], 'Loading sources…')) ?></span>
                            </div>
        ...
        <?php if ($type !== 'tv'): ?>
        <div class="aside-wrapper swap mt-4 watch-movie-info-row">
            <aside class="main">
                <section id="w-info">
                    <div class="rating-box" data-score="<?= e((string) ($item['vote_average'] ?? 0)) ?>" data-id="<?= (int) $item['id'] ?>">
                        <?php if ($rating !== null): ?>
            ...
            <?php if ($similar): ?>
            <aside class="sidebar watch-similar-sidebar">
