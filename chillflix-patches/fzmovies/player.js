...
<div class="np-shell ${cfg.embed ? "np-embed" : ""}" id="np-shell">
  ...
  function createPlayer(cfg) {
    ...
    function destroyHls() {
      if (state.hls) {
        try {
          state.hls.detachMedia && state.hls.detachMedia();
        } catch (_) {}
        try {
          state.hls.destroy();
        } catch (_) {}
      ...
      // Always pause/clear the element — HLS destroy alone can leave buffered audio
      // briefly, and native HLS (Safari) has no hls.js instance to tear down.
      hardStopVideo(els.video);
    }

    function killCardPreviews() {
      try {
        if (typeof window.__cfKillCardPreviews === "function") {
          window.__cfKillCardPreviews();
        }
      } catch (_) {}
      try {
        document.querySelectorAll("video.cf-card-video").forEach((video) => {
          try {
            video.muted = true;
            video.pause();
            video.removeAttribute("src");
    ...
    async function playUrl(url, source) {
      ...
      if (!isCineplayYoruSource(url, source) && candidateUrls(source).length > 1) {
        ...
        if (playGen !== state.playGen) return;
      ...
      if (isHls && window.Hls && window.Hls.isSupported()) {
        ...
        hls.on(window.Hls.Events.MANIFEST_PARSED, (_e, data) => {
          if (playGen !== state.playGen || state.hls !== hls) return;
        ...
        // Keep per-URL quality packs (FzMovies/HollyBox/file) — do not wipe for native HLS.
        if (!applySourceQualities(source)) {
          state.qualityOptions = [];
        }
        refreshMenus();
        ...
        state.audioTracks = fallbackAudioTracks(source);
        state.audioIndex = preferEnglishAudioIndex(state.audioTracks, source?.preferAudio || source?.language || "en");
        // Progressive MP4/MKV (FzMovies etc.): keep explicit qualities for the Quality menu.
        if (!applySourceQualities(source)) {
          state.qualityOptions = [];
        }
        refreshMenus();
    ...
    async function fetchProviderSourcesOnce(pid, slot) {
      ...
      if (pid === "torrentio") timeoutSec = Math.max(35, Math.min(timeoutSec, 90));
      if (pid === "yesmovies") timeoutSec = Math.max(35, Math.min(timeoutSec, 90));
     ...
     */
    function racePackPatienceMs() {
      // Busy scrapes need runway — do not abandon the first pack too early.
      return 45000;
    }

    function racePackStillLoading() {
      const pack = Array.isArray(state.racePackIndices) ? state.racePackIndices : [];
      return pack.some((idx) => {
        const slot = state.sources[idx];
        if (!slot) return false;
        const pid = String(slot.provider || "").toLowerCase();
        if (pid && state.providerLoads[pid]) return true;
        return slot.status === "loading" || slot.status === "pending";
      });
    }

    function racePackReadyIndex() {
      const pack = Array.isArray(state.racePackIndices) ? state.racePackIndices : [];
      const tried = state.raceTried || new Set();
      for (const idx of pack) {
        if (tried.has(idx)) continue;
        const slot = state.sources[idx];
        if (!slot) continue;
        if (slot.hasEnglish === false && !slot.autoloadNonEnglish) continue;
        if (
          slot.status === "ready" &&
          (slot.url || streamQualitiesFromSource(slot).length || candidateUrls(slot).length)
        ) {
          return idx;
        }
      }
    ...
    function clearRacePackTimer() {
      if (state.racePackTimer) {
        try {
          clearTimeout(state.racePackTimer);
        } catch (_) {}
        state.racePackTimer = null;
      }
    }

    /** Prefer another ready race-pack source, or wait while pack scrapes are still in flight. */
    function playNextRacePackSource() {
      const readyIdx = racePackReadyIndex();
      if (readyIdx >= 0) {
        if (state.raceTried) state.raceTried.add(readyIdx);
        state.raceWinnerIndex = readyIdx;
        startPlaybackForIndex(readyIdx);
        return true;
      }
      if (state.raceActive || racePackStillLoading()) {
        const started = state.raceStartedAt || Date.now();
        const left = Math.max(0, racePackPatienceMs() - (Date.now() - started));
        if (left > 0) {
          const names = (state.racePackIndices || [])
            .map(
              (i) =>
                state.sources[i]?.publicLabel ||
                state.sources[i]?.providerName ||
                state.sources[i]?.provider ||
                "?"
            )
            .join(", ");
          setStatus(`Still racing ${names}… ${Math.ceil(left / 1000)}s`);
          clearRacePackTimer();
          state.racePackTimer = setTimeout(() => {
            state.racePackTimer = null;
            if (mediaLooksPlayable(els.video)) return;
            if (!playNextRacePackSource()) {
              // Patience exhausted and nothing ready — fall through sequential cascade.
              const pack = state.racePackIndices || [];
              const after = pack.length ? Math.max(...pack) + 1 : state.sourceIndex + 1;
              for (let i = after; i < state.sources.length; i++) {
                if (state.sources[i]?.autoLoad === false) continue;
                ensureProviderLoaded(i, { play: true, isAuto: true });
                return;
              }
    ...
    async function raceAutoSources({ play = true, background = false, packSize = 4 } = {}) {
      ...
      state.racePackIndices = pack.slice();
      state.raceTried = new Set();
      ...
      await Promise.all(
        pack.map(async (idx) => {
          ...
          if (state.raceTried) state.raceTried.add(idx);
          if (background && state.cascadeExhausted) {
      ...
      if (readyIdx >= 0) {
        state.raceWinnerIndex = readyIdx;
        if (state.raceTried) state.raceTried.add(readyIdx);
        if (play) startPlaybackForIndex(readyIdx);
        return true;
      }
    ...
    async function ensureProviderLoaded(index, { play = false, isAuto = false, background = false, claimIndex = true, quietStatus = false, raceLabel = "" } = {}) {
      ...
      slot.status = "loading";
      refreshMenus();
      if (!quietStatus && (!background || !state.cascadeExhausted)) {
        setStatus(`Fetching ${slot.providerName || pid}…`);
      } else if (quietStatus && raceLabel && (!background || !state.cascadeExhausted)) {
        setStatus(`Racing ${raceLabel}…`);
      }
      ...
      try {
        ...
        const softPid = pid === "moonflix" || pid === "hdghar" || pid === "bingr" || pid === "opstream" || pid === "vsembed" || pid === "torrentio" || pid === "yesmovies";
        if (!emptyish && softPid) {
          slot.status = "pending";
    ...
    function armLoadWatchdog(source) {
      ...
      } else if (viaProxy || pid === "vsembed" || pid === "opstream" || pid === "torrentio" || pid === "yesmovies" || pid === "onlyflix" || pid === "moviebox" || pid === "megasource" || pid === "vaplayer") {
        // First-pack providers: give cold CDNs ~30s before cascading.
        waitMs = Math.max(30000, Math.min(75000, Math.max(base, 30000)));
      } else {
        waitMs = Math.max(12000, Math.min(45000, base));
      }
    ...
    function tryNextSource(opts) {
      opts = opts || {};
      const slot = state.sources[state.sourceIndex];
      // During / after the first race pack: prefer other pack hits and wait for slow scrapes
      // instead of instantly cascading past Sigma/Mega/Pi/Alpha.
      if (Array.isArray(state.racePackIndices) && state.racePackIndices.length) {
        if (state.raceTried) state.raceTried.add(state.sourceIndex);
        if (playNextRacePackSource()) return;
      }
      const waitMs = Math.max(sourceAutoWaitMs(slot), 25000);
      const started = state.sourcePlayStartedAt || 0;
      const elapsed = started ? Date.now() - started : waitMs;
      // Hard miss / empty: skip Auto-wait and cascade immediately — but never while race pack still loading.
      const skipWait = !!opts.immediate || slot?.status === "empty";
      if (!skipWait && started && elapsed < waitMs) {
        const left = Math.max(250, waitMs - elapsed);
        setStatus(
          `Waiting on ${slot?.providerName || slot?.provider || "source"}… ${Math.ceil(left / 1000)}s`
        );
    ...
    return {
      destroy() {
        // Invalidate in-flight playUrl / HLS callbacks so they cannot restart audio
        state.playGen += 1;
  ...
  window.ChillflixPlayer = {
    ...
    startOnWatch(cfg) {
      ...
      // Kill any leftover unmuted card previews before mounting
      try {
        if (typeof window.__cfKillCardPreviews === "function") window.__cfKillCardPreviews();
      } catch (_) {}
