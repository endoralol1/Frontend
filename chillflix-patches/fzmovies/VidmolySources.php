...
final class VidmolySources
{
     ...
     * @param array<string,string> $headers
     */
    private static function proxyStreamBinary(string $url, array $headers, bool $insecureTls = false): void
    {
        ...
        if (self::shouldUseWorkerEgress($url)) {
            $relay = '';
            $secret = '';
            ...
            if ($relay === '') {
                $relay = rtrim((string) (getenv('YORU_RELAY_URL') ?: ''), '/');
                $secret = (string) (getenv('YORU_RELAY_SECRET') ?: '');
            }
            if ($relay !== '') {
                $fetchUrl = $relay . '/stream?url=' . rawurlencode($url);
                if ($secret !== '') {
                    $fetchUrl .= '&key=' . rawurlencode($secret);
                    $hdrLines[] = 'X-Yoru-Key: ' . $secret;
                }
            }
        }
     ...
     */
    private static function proxyFetch(string $url, array $headers, bool $insecureTls = false): ?array
    {
        if (!function_exists('curl_init')) {
            return null;
        }
        if (self::shouldUseWorkerEgress($url)) {
            $via = self::workerFetch($url, $headers, false);
            if (is_array($via) && (int) ($via['status'] ?? 0) < 400) {
                return $via;
            }
        }
        $hdrLines = ['User-Agent: ' . self::UA];
    ...
    private static function shouldUseWorkerEgress(string $url): bool
    {
        if (class_exists('NetMirrorSources') && NetMirrorSources::needsWorkerEgress($url)) {
            return true;
        }
        $host = strtolower((string) (parse_url($url, PHP_URL_HOST) ?: ''));
        return $host !== '' && preg_match('/^net\d{2,3}\.cc$/', $host) === 1;
    }

    /**
     * Fetch via Yoru Worker /fetch (text/playlist) or /stream (binary).
     * Requires net*.cc on the Worker allowlist (yoru-relay.js).
     *
     * @param array<string,string> $headers
     * @return array{status:int,body:string,contentType:string,contentLength:string}|null
     */
    private static function workerFetch(string $url, array $headers, bool $binary): ?array
    {
        $relay = '';
        $secret = '';
        if (class_exists('WorkerRelayService')) {
        ...
        if ($binary) {
            $endpoint = $relay . '/stream?url=' . rawurlencode($url);
            if ($secret !== '') {
                $endpoint .= '&key=' . rawurlencode($secret);
            }
            $hdrLines = [];
            foreach ($headers as $k => $v) {
                $hdrLines[] = $k . ': ' . $v;
            ...
            $errno = curl_errno($ch);
            curl_close($ch);
            if ($body === false || $errno || $status >= 400) {
                return null;
            }
            return [
                'status' => $status,
                'body' => (string) $body,
                'contentType' => $ctype !== '' ? $ctype : 'application/octet-stream',
                'contentLength' => (string) strlen((string) $body),
            ];
        }

        $endpoint = $relay . '/fetch?url=' . rawurlencode($url);
        if ($secret !== '') {
            $endpoint .= '&key=' . rawurlencode($secret);
        }
        $ch = curl_init($endpoint);
        ...
        $errno = curl_errno($ch);
        curl_close($ch);
        if (!is_string($raw) || $raw === '' || $errno) {
            return null;
        }
        $json = json_decode($raw, true);
        if (!is_array($json) || empty($json['ok']) || !isset($json['body'])) {
            return null;
        }
        ...
        return [
            ...
            'body' => $body,
            'contentType' => (string) ($json['contentType'] ?? 'application/vnd.apple.mpegurl'),
