...
final class PlayerSources
{
    ...
    public static function fetch(string $type, int $tmdbId, int $season = 1, int $episode = 1, ?string $onlyProvider = null): array
    {
        ...
        $localProviderIds = ['stremify', 'nxsha', 'castle', 'awsind', 'nitro', 'riveprime', 'hdghar', 'moonflix', 'hollybox', 'moviebox', 'flixhqz', 'cinejoy', 'novahd', 'netmirror', 'megasource', 'opstream', 'torrentio', 'yesmovies', 'fzmovies', 'videasy', 'ridomovies', 'filesun', 'bingr', 'moviesonlinehd', 'vsembed', 'cineplay', 'vidking', 'vidmoly', 'upcloud', 'byse'];
        ...
        foreach ($providers as $provider) {
            ...
            // Videasy (Cinemove VE) — speedracelight multi-server.
            if ($provider === 'videasy') {
                ...
                foreach ($ve['sources'] ?? [] as $src) {
                    ...
                    $streamUrl = (string) $src['url'];
                    $quality = (string) ($src['quality'] ?? 'Auto');
                    $hasPack = !empty($src['qualities']) && is_array($src['qualities']);
                    $key = 'videasy|' . ($hasPack ? 'pack' : $streamUrl);
                    if (isset($merged[$key])) {
                        continue;
                    }
                    $merged[$key] = [
                        ...
                        'type' => (string) ($src['type'] ?? 'hls'),
                        'quality' => $quality !== '' ? $quality : 'Auto',
                        'qualities' => $hasPack ? $src['qualities'] : [],
                        'provider' => 'videasy',
            ...
            // YesMovies → ployan.me direct HLS (PBKDF2 token, no browser).
            if ($provider === 'yesmovies') {
                if (!class_exists('YesMoviesSources')) {
                    $diagnostics[] = [
                        'code' => 'YESMOVIES_MISSING',
                        'message' => 'YesMoviesSources class not loaded',
                        'severity' => 'warning',
                        'provider' => 'yesmovies',
                    ];
                    continue;
                }
                $ym = YesMoviesSources::fetch($type, $tmdbId, $season, $episode);
                foreach ($ym['diagnostics'] ?? [] as $d) {
                    if (is_array($d)) {
                        $diagnostics[] = $d;
                    }
                }
                foreach ($ym['sources'] ?? [] as $src) {
                    if (!is_array($src) || empty($src['url'])) {
                        continue;
                    }
                    $streamUrl = (string) $src['url'];
                    $quality = (string) ($src['quality'] ?? 'Auto');
                    $key = 'yesmovies|' . $streamUrl;
                    if (isset($merged[$key])) {
                        continue;
                    }
                    $merged[$key] = [
                        ...
                        'quality' => $quality !== '' ? $quality : 'Auto',
                        'provider' => 'yesmovies',
                        'providerName' => (string) ($src['providerName'] ?? 'YesMovies'),
                        'label' => (string) ($src['label'] ?? ('YesMovies · ' . $quality)),
                        'language' => (string) ($src['language'] ?? 'en'),
                        'hasEnglish' => !empty($src['hasEnglish']),
                        'audioTracks' => [],
                        'meta' => is_array($src['meta'] ?? null) ? $src['meta'] : [],
                ...
                if (empty($ym['ok']) && empty($ym['sources'])) {
                    $diagnostics[] = [
                        'code' => 'YESMOVIES_EMPTY',
                        'message' => (string) ($ym['error'] ?? 'No YesMovies streams'),
                        'severity' => 'warning',
                        'provider' => 'yesmovies',
                    ];
                }
