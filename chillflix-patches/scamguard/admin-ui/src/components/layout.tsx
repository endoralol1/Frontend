...
export function AdminLayout({ username, children }: { username: string; children?: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-7xl flex-col gap-0 md:flex-row md:gap-6 md:p-6">
      <aside className="w-full shrink-0 border-b border-border bg-card/80 backdrop-blur md:w-60 md:rounded-2xl md:border md:shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 md:block">
          <div>
            <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">ScamGuard</div>
            <div className="mt-1 text-lg font-semibold tracking-tight">Admin</div>
          </div>
