...
<div class="np-shell ${cfg.embed ? "np-embed" : ""}" id="np-shell">
  ...
  function createPlayer(cfg) {
    ...
    const state = {
      ...
      // Progressive sources: only scrape admin-first (or selected) provider.
    ...
    async function playUrl(url, source) {
      ...
      if (isHls && window.Hls && window.Hls.isSupported()) {
        ...
        const stallTimer = setTimeout(() => {
          ...
          setStatus("Stream stalled — try another audio/source…");
          tryNextSource();
        }, stallMs);
        v.addEventListener(
          "loadedmetadata",
    ...
    async function fetchProviderSourcesOnce(pid, slot) {
      ...
      q.set("provider", pid);
      const timeoutMs = Math.max(15, Math.min(180, Number(slot?.scrapeTimeoutSec) || 45)) * 1000;
      const ctrl = new AbortController();
      ...
      if (!data?.ok || !Array.isArray(data.sources) || !data.sources.length) {
        throw new Error(data?.error || `No streams from ${slot.providerName || pid}`);
      }
    ...
    async function ensureProviderLoaded(index, { play = false, isAuto = false } = {}) {
      ...
      const slowProviders = new Set(["vsembed", "flixhqz", "cineplay", "vidmoly", "hdghar", "bingr", "castle", "awsind"]);
      ...
      try {
        ...
        const emptyish = /no source|not available|empty|no playable|no streams|no hdghar|no holly|HDGHAR_EMPTY/i.test(String(err?.message || ""));
        slot.status = emptyish ? "empty" : "error";
        slot.error = err?.message || t('common.failed', 'Failed');
        refreshMenus();
        if (isAuto || play) {
          // Auto / failure path: try next admin-ordered source (don't scrape all up front).
          const next = idx + 1;
          if (next < state.sources.length) {
            setStatus(`${slot.providerName || pid} ${emptyish ? "no source" : "failed"} — trying next…`);
            return ensureProviderLoaded(next, { play: true, isAuto: true });
          }
          setStatus(err?.message || t('player.no_sources', 'No playable sources'));
        } else {
          setStatus(emptyish ? (err?.message || "No source for this title") : (err?.message || "Source failed"));
        }
        return false;
      } finally {
    ...
    function tryNextSource() {
      ...
      // If playback was attempted, respect Auto wait before cascading.
      if (started && elapsed < waitMs) {
        ...
        state.autoWaitTimer = setTimeout(() => {
          ...
          } else setStatus(t('player.no_sources', 'No playable sources'));
        }, left);
        return;
      }
      if (state.sourceIndex + 1 < state.sources.length) {
    ...
    async function fetchSources() {
      ...
      try {
        ...
        await ensureProviderLoaded(0, { play: true, isAuto: true });
      } catch (err) {
