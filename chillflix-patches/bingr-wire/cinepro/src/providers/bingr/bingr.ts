...
function qualityRank(q: string): number {
    ...
    if (/auto|unknown|^$/.test(lower)) return 1080;
    const n = parseInt(q, 10);
    return Number.isFinite(n) ? n : 0;
}

function cleanQuality(raw: string | undefined): string {
    const q = String(raw || '').trim();
    if (!q || /^(auto|unknown|default)$/i.test(q)) return 'Auto';
...
function languageScore(lang: string | undefined, label: string | undefined): number {
    const blob = `${lang || ''} ${label || ''}`.toLowerCase();
    if (/\benglish\b|\beng\b/.test(blob) && !/\bhindi\b/.test(blob)) return 100;
    if (/\bmulti\b/.test(blob)) return 95;
    if (/\boriginal\b/.test(blob)) return 85;
    if (/\bhindi\b/.test(blob)) return 55;
    if (blob.trim()) return 50;
    return 70;
}
 ...
 */
function unwrapWormhole(url: string): { url: string; headers: Record<string, string> } | null {
    try {
        const u = new URL(url);
        if (!/wormhole\.filmu\.in$/i.test(u.hostname)) return null;
        if (!/\/proxy\/(m3u8|file)/i.test(u.pathname)) return null;
        const nested = u.searchParams.get('url');
        if (!nested || !/^https?:\/\//i.test(nested)) return null;
        ...
        if (origin && !headers.Origin) headers.Origin = origin;
...
function pickBest(sources: BingrSource[]): BingrSource | null {
    const usable = sources.filter((s) => /^https?:\/\//i.test(String(s.url || '').trim()));
    if (!usable.length) return null;
    usable.sort((a, b) => {
        const ls =
            languageScore(b.language, b.label || b.name) -
...
export class BingrProvider extends BaseProvider {
    ...
    private async postStream(
        ...
        try {
            ...
            let json: BingrStreamResponse = {};
            try {
                json = JSON.parse(text) as BingrStreamResponse;
            } catch {
    ...
    private mapSubtitles(
        raw: BingrStreamResponse['subtitles'] | undefined
    ): Subtitle[] {
    ...
    private buildPlayable(hit: BingrSource, scraperName: string): Source {
    ...
    private async resolve(media: ProviderMediaObject): Promise<ProviderResult> {
        ...
        try {
            ...
            return this.emptyResult(
                lastError || `No Bingr streams (tried ${tried.join(',')})`
            );
        } catch (error) {
            return this.emptyResult(
