...
import { buildCineproProxyHref, compactCineproProxyHref } from "@/lib/cinepro/proxy"
...
import {
    buildHuhuLanguageStreamSources,
    buildHuhuStreamUrl,
...
function finalizeBackgroundCache(args: {
  ...
  // VidLink / FlixHQ / etc. already returned — don't stamp a scary
  // "CinePro did not return" warning over a working stream. Extra providers
  // simply stayed empty; that is not a user-facing failure.
  const sourceCount = Array.isArray(record.sources) ? record.sources.length : 0
  const total = typeof record.total === "number" ? record.total : sourceCount
...
function buildMergedResult(args: {
  ...
  const nativeSources = args.requestedProvider
    ? []
    : [args.vidlinkSource, args.flixhqSource, ...args.huhuSources].filter(
...
function normalizeSources(rawSources: SourceItem[] = [], cineproBaseUrl: string, origin: string) {
  ...
  return rawSources
    ...
    .map((source) => {
      let rewritten = rewriteCineproProxyUrl(source.url, cineproBaseUrl, origin)
      const providerId = normalizeProviderName(
        source.provider?.id ?? source.provider?.name ?? "unknown"
      )
      // VSEmbed CDN JWTs are VPS IP-bound — tunnel via media proxy (no Referer; CF 403 otherwise).
      if (
        providerId === "vsembed" &&
        !rewritten.includes("/api/cinepro/proxy") &&
        !rewritten.includes("/v1/proxy")
      ) {
        rewritten = buildCineproProxyHref(origin, source.url, {
          "User-Agent":
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36",
        })
      }
      // Cinejoy (earthcleaner) HLS requires Referer https://cinejoy.to/ or CDN returns 418.
      if (
        providerId === "cinejoy" &&
        !rewritten.includes("/api/cinepro/proxy") &&
        !rewritten.includes("/v1/proxy")
...
async function respondNativeHuhuRequest(args: {
  ...
  const huhuSources = (await buildHuhuLanguageStreamSources({
    requestOrigin: args.requestOrigin,
    type: args.type,
    tmdbId: args.tmdbId,
    season: args.season,
  ...
  const languageLabels = configuredSources
    ...
    diagnostics: [
      {
        ...
        message: args.nativeHuhuEnabled
          ? `Native Huhu languages ready: ${languageLabels || "English"}.`
  ...
  try {
    ...
    const huhuSources =
      nativeHuhuEnabled && huhuEnabled
        ? ((await buildHuhuLanguageStreamSources({
            requestOrigin,
            type,
            tmdbId,
            season: season ?? undefined,
    ...
    const huhuDiagnostics: Array<{
      ...
      nativeHuhuEnabled && huhuEnabled
        ? [
            {
              code: "HUHU_SOURCE_ADDED",
              message: `Native Huhu languages ready: ${
                huhuSources.map((source) => source.quality).join(", ") || "English"
              }.`,
              severity: "info",
            },
          ]
    ...
    if (!cineproBaseUrl) {
      const nativeSources = collectNativeSources({ vidlinkSource, flixhqSource, huhuSources })
    ...
    if (vidlinkSource || flixhqSource || huhuSources.length) {
    ...
    if (!payload) {
      const fallbackSources = collectNativeSources({ vidlinkSource, flixhqSource, huhuSources })
      const result = finalizeSourcePayload(
        {
