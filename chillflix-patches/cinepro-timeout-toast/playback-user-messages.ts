...
/**
 * Soft timeouts for *extra* CinePro providers. Harmless when VidLink / another
 * source already returned a stream — never show these as a Retry CTA over video.
 */
const SOFT_EXTRA_SOURCE_TIMEOUT_CODES = new Set(["CINEPRO_UNAVAILABLE"])
...
export function isSoftExtraSourceTimeoutDiagnostic(code?: string) {
    return Boolean(code && SOFT_EXTRA_SOURCE_TIMEOUT_CODES.has(code))
}

export function findInfrastructurePlaybackMessage(
    diagnostics: PlaybackDiagnostic[],
    opts?: { ignoreSoftTimeouts?: boolean }
) {
    return diagnostics.find((item) => {
        if (!isPlaybackInfrastructureDiagnostic(item.code)) return false
        if (opts?.ignoreSoftTimeouts && isSoftExtraSourceTimeoutDiagnostic(item.code)) {
            return false
        }
        return true
    })?.message
...
export function pickUserFacingPlaybackWarning(args: {
    ...
    // Soft "extra sources timed out" must not cover an already-playing stream.
    const infraMessage = findInfrastructurePlaybackMessage(args.diagnostics, {
        ignoreSoftTimeouts: args.hasPlayableSource,
    })
