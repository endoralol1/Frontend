#!/usr/bin/env node
/**
 * Resolve Videasy (Cinemove "VE") servers via api.speedracelight.com.
 * Same enc=2 + seed decrypt as player.videasy.to / vidking.net.
 *
 * Races English-friendly Videasy servers (Yoru/cdn, Neon/vsrc, Breach/m4uhd,
 * Cypher/downloader2, Vyse/hdmovie-en) and returns the first pack with streams.
 *
 * Usage:
 *   node videasy-resolve.mjs --type movie --tmdb 157336 [--title "Interstellar"] [--year 2014]
 *   node videasy-resolve.mjs --type tv --tmdb 1396 --season 1 --episode 1
 */
let ProxyAgent = null;
...
async function main() {
  ...
  try {
    ...
    // Videasy player server map (English-first). Paths match player.videasy.to chunk 8351.
    const SERVERS = [
      { name: "Yoru", path: "cdn", filter: null },
      { name: "Neon", path: "vsrc", filter: null },
      { name: "Breach", path: "m4uhd", filter: null },
      { name: "Cypher", path: "downloader2", filter: null },
      { name: "Vyse", path: "hdmovie", filter: (s) => String(s.quality || "") === "English" },
    ];
    ...
    async function tryServer(server) {
      ...
      for (const p of attempts) {
        try {
          const r = await httpGet(
            `${API}/${server.path}/sources-with-title?${qs}`,
            headers,
            p || undefined
          );
          if (!r.ok) {
            lastErr = `${server.name} ${r.status}`;
            continue;
          }
          blob = r.text;
          break;
      ...
      let sources = (parsed.sources || []).filter((s) => s && s.url);
      if (server.filter) sources = sources.filter(server.filter);
      // Prefer HLS / drop dash-only when mixed
      const hls = sources.filter(
        (s) =>
          String(s.type || "").toLowerCase() === "hls" ||
          /\.m3u8(\?|$)/i.test(String(s.url || ""))
      );
      if (hls.length) sources = hls;
    ...
    // Race all servers; take first non-empty
    const raced = await Promise.all(SERVERS.map((s) => tryServer(s).catch(() => null)));
    const winner = raced.find((r) => r && r.sources && r.sources.length);
    if (!winner) {
      console.log(
        JSON.stringify({
          ok: false,
          error: lastErr || "all videasy servers empty",
          cfBlocked: /429|403|1015/.test(String(lastErr)),
        })
      );
      process.exit(2);
    }
