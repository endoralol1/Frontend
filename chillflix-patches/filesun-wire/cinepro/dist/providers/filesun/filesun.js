...
function extractM3u8(html) {
    const fromSources = html.match(/sources\s*:\s*\[\s*\{\s*file\s*:\s*['"]([^'"]+\.m3u8[^'"]*)['"]/i)?.[1];
    if (fromSources)
        return fromSources.replace(/&amp;/g, '&');
...
export class FilesunProvider extends BaseProvider {
    ...
    async resolveImdbId(media) {
        const direct = String(media.imdbId || '').trim();
        if (/^tt\d+$/i.test(direct))
            return direct;
        const apiKey = process.env.TMDB_API_KEY?.trim();
        if (!apiKey || !media.tmdbId)
            return null;
    ...
    async resolveFromEmbed(embedPath) {
        ...
        let servers = [];
        try {
            const json = JSON.parse(resolveRes.body);
            if (json.error)
                throw new Error(String(json.error));
            servers = Array.isArray(json.servers) ? json.servers : [];
        }
        catch (error) {
            if (error instanceof Error && error.message.startsWith('Resolve'))
                throw error;
        ...
        for (const server of servers.slice(0, 4)) {
            ...
            if (!emb.ok || /just a moment/i.test(emb.body))
                continue;
    ...
    async resolve(media) {
        ...
        try {
            ...
            const imdbId = media.type === 'tv' ? null : await this.resolveImdbId(media);
            ...
            if (!path)
                return this.emptyResult('Could not build embed path');
            const hit = await this.resolveFromEmbed(path);
            if (!hit)
                return this.emptyResult('No playable FileSuN servers');
            const source = {
                url: hit.url,
                type: 'hls',
                quality: hit.quality,
...
//# sourceMappingURL=filesun.js.map